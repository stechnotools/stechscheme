            ...
            <Stack spacing={3}>
              <Stack direction={{ xs: 'column', md: 'row' }} justifyContent='space-between' spacing={2}>
                <div>
                  ...
                  <Typography color='text.secondary'>
                    Track customer scheme enrollments with installment and payment summaries.
                  </Typography>
                </div>
                <Button component={Link} href='/membership/create' variant='contained'>
                  Create Membership
                </Button>
              </Stack>
              ...
              {loading ? (
                ...
                <Grid container spacing={3}>
                        ...
                        <Card variant='outlined'>
                          <CardContent>
                            <Stack spacing={2}>
                              <Stack direction='row' justifyContent='space-between' spacing={2}>
                                <div>
                                  ...
                                  <Typography variant='body2' color='text.secondary'>
                                    {`${item.scheme?.name || 'No scheme'} • ${item.scheme?.code || 'No code'}`}
                                  </Typography>
                                </div>
                              ...
                              <Typography variant='body2' color='text.secondary'>
                                {`Start ${new Date(item.start_date).toLocaleDateString('en-IN')} • Maturity ${new Date(item.maturity_date).toLocaleDateString('en-IN')}`}
                              </Typography>

                              <Typography variant='body2' color='text.secondary'>
                                {`Installments ${paidInstallments}/${totalInstallments} • Payments ${item.payments?.length || 0}`}
                              </Typography>
                              ...
                              <Stack direction='row' justifyContent='flex-end'>
                                <Button component={Link} href={`/membership/${item.id}`} variant='text'>
                                  View Details
                                </Button>
                              </Stack>
                            </Stack>
                  ...
                  {filtered.length === 0 ? (
                    <Grid size={{ xs: 12 }}>
                      <Alert severity='info'>No memberships found.</Alert>
                    </Grid>
                  ) : null}
                </Grid>
              )}
