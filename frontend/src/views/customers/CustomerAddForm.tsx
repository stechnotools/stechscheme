...
export interface CustomerAddFormHandle {
  submit: () => Promise<void>
}

export const CustomerAddForm = forwardRef<CustomerAddFormHandle, CustomerAddFormProps>(
  ({ onSuccess, onCancel, embedded = false, fullFields = false }, ref) => {
  ...
  const [showAdvanced, setShowAdvanced] = useState(false)
    ...
    return (
      <Grid container spacing={embedded ? 4 : 6}>
        {!embedded && (
          <Grid size={{ xs: 12 }}>
            <Card
              sx={{
        ...
        <Grid size={{ xs: 12, lg: (embedded || !showAdvanced) ? 12 : 8 }}>
          <Stack spacing={4}>
            {error ? <Alert severity='error'>{error}</Alert> : null}
            {success ? <Alert severity='success'>{success}</Alert> : null}

            {/* Basic Details */}
            <Card elevation={embedded ? 0 : undefined} variant={embedded ? 'outlined' : undefined}>
              <CardContent>
                <Stack spacing={3}>
            ...
            <Box sx={{ py: 2 }}>
              <Button
                ...
                startIcon={<i className={showAdvanced ? 'ri-subtract-line' : 'ri-add-line'} />}
                sx={{ fontWeight: 600 }}
              >
                {showAdvanced ? 'Hide Advanced Information' : 'Show Advanced Information (KYC, Nominee, Documents)'}
              </Button>
            </Box>
                  ...
                  <CardContent>
                    <Stack spacing={3}>
                      ...
                      <Grid container spacing={3}>
                        ...
                        <Grid size={{ xs: 12, md: 6 }}>
                          <TextField fullWidth label='City' value={nomineeCity} onChange={e => setNomineeCity(e.target.value)} />
                        </Grid>
                        <Grid size={{ xs: 12, md: 4 }}>
...
CustomerAddForm.displayName = 'CustomerAddForm'
